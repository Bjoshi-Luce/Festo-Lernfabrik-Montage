...
{
    /// <summary>
    /// Offers the possibility to download the VR Builder Core package.
    /// </summary>
    internal class CoreDownloadWindow : EditorWindow
    {
        ...
        private bool canDownloadRequired;
        ...
        private void OnGUI()
        {
            ...
            titleContent = new GUIContent("Core Package Manager");
            ...
            {               
                ...
                canDownloadRequired = string.IsNullOrEmpty(CoreVersionChecker.RequiredVersion) == false && CoreVersionChecker.RequiredVersion != "unknown" && isRequiredVersionAvailable && CoreVersionChecker.RequiredVersion != latestVersion;
                canDownloadLatest = string.IsNullOrEmpty(latestVersion) == false && latestVersion != "unknown";

                if (canDownloadLatest && GUILayout.Button(string.Format("Install version {0} (latest)", latestVersion))) 
                {
                    if (DisplayBackupDialog())
                    {
                ...
                showMoreOptions = EditorGUILayout.Foldout(showMoreOptions, "More download options");

                EditorGUI.BeginDisabledGroup(isDownloading || isCoreAvailable == false);

                if (showMoreOptions)
                {
                    if (canDownloadRequired && GUILayout.Button(string.Format("Install version {0} (required)", CoreVersionChecker.RequiredVersion)))
                    {
                        if (DisplayBackupDialog())
                        {
        ...
        private bool DisplayBackupDialog()
        {
            return EditorUtility.DisplayDialog("Backup Warning", string.Format("Importing VR Builder will also import some Unity packages as dependencies and switch to the new input system.\n\n" +
                "It is recommended to make a backup when importing it in an existing project."), "Go ahead!", "Cancel");
        }
    }
}